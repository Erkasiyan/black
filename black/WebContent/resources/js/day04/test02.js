...
document.getElementById('rbtn').onclick = function(){
	...
	if(sid){
		...
		// sid에 데이터 채워져 있지 않은 경우(null 또는 undefined 인 경우)
...
document.getElementById('lbtn').onclick = function(){
	...
	if(result){
		// 패턴에 맞는 경우
		console.log('입력한 아이디 : ' + sid);
	} else{
		// 패턴에 맞지 않은 경우
		document.getElementById('tid').value = '';
		console.log('형식에 맞지 않은 아이디입니다.');
	}
};
*/
...
// 유효성 검사를 하고
// 검사에 통과하면 test.html 로 아이디와 비밀번호를 전달하세요.
// 통과하지 않은 경우는 입력내용을 모두 지우고 다시 입력하도록 하세요.

// 비밀번호는 첫문자는 알파벳이어야 하고
// 숫자와 알파벳으로 구성된 4글자이상 8글자 이하로 입력해야 한다.
...
document.getElementById('lbtn').onclick = function(){
	...
	if(idResult && pwResult){
		...
		documnet.getElementById('tid').focus();
	}
